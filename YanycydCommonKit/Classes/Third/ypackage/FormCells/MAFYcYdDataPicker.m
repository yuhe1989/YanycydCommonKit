...
@interface MAFYcYdDataPicker () <UIPickerViewDelegate, UIPickerViewDataSource>
@property (nonatomic, strong) UIView *blackBG;
@property (nonatomic, strong) UIView *contentView;
@property (nonatomic, strong) UIPickerView *pickerView;
@end

@implementation MAFYcYdDataPicker
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if ([self.type isEqualToString:@"chulifangshi"]) {
        return _dataArr[row];
    }
    
    MAFYcYdDataModel *model = _dataArr[row];
...
- (void)btnAction:(UIButton *)sender {
    if (sender.tag == 0) {
        ...
        if ([self.type isEqualToString:@"chulifangshi"]) {
            NSString * str = [_dataArr objectAtIndex:[_pickerView selectedRowInComponent:0]];
            if (_delegate && [_delegate respondsToSelector:@selector(didSelectString:inView:)]) {
                [_delegate didSelectString:str inView:self];
            }
        }else{
            MAFYcYdDataModel *model = [_dataArr objectAtIndex:[_pickerView selectedRowInComponent:0]];
