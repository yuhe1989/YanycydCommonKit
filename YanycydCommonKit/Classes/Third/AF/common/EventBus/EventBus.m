...
@property (nonatomic, strong) NSMutableDictionary * subscriberList;
...
- (id)init
{
    self = [super init];
    if(self){
        _subscriberList = [NSMutableDictionary dictionary];
        _eventList = [NSMutableArray arrayWithCapacity:EVENT_COUNT];
    }
    return self;
}
...
- (void)_addEvent: (NSString *)eventName eventData: (id)eventData publisher: (id<EventPublisher>)publisher
{
    if([publisher conformsToProtocol:@protocol(EventAsyncPublisher)]){
        ...
        AsyncEvent * event = [[AsyncEvent alloc] init];
